...
def register_office_views(app):
  ...
  def office():
    ...
    scene = (
        ...
        '<script>fetch("/api/logs").then(r=>r.ok?r.json():Promise.reject()).then(logs=>{const board=document.querySelector("#office-live-status");if(!logs.length){board.innerHTML="<b>実データ</b><span>表示できる作業ログはまだありません。</span>";return;}const latest=logs[0];board.innerHTML="<b>実データ</b><span>最新ログ："+latest[2]+"（"+latest[4]+"）</span>";}).catch(()=>{document.querySelector("#office-live-status").innerHTML="<b>実データ</b><span>作業ログを取得できませんでした。</span>";});</script>'
